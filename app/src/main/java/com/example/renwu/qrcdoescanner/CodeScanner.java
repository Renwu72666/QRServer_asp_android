package com.example.renwu.qrcdoescanner;
...
public class CodeScanner {
	...
	public static interface CodeReaderListener {
		...
		CodeReaderListener codeReaderListener = new CodeReaderListener(){
			@Override
			public void codeReadResult(final String type, final String data) {
	...
	public boolean scan() {
		...
		final Intent easyBarcodeScanner = new Intent("com.google.zxing.client.android.SCAN");
		easyBarcodeScanner.setPackage("org.magiclen.barcodescanner");
		final List<ResolveInfo> list = packageManager.queryIntentActivities(easyBarcodeScanner, PackageManager.GET_ACTIVITIES);
		if (list.size() > 0) {
			...
			new AlertDialog.Builder(activity).setTitle(getString(STRING_INSTALL_TITLE)).setMessage(getString(STRING_INSTALL_MESSAGE)).setPositiveButton(getString(STRING_INSTALL_INSTALL), new DialogInterface.OnClickListener() {
				...
				public void onClick(final DialogInterface dialog, final int which) {
					...
					final List<ResolveInfo> list = packageManager.queryIntentActivities(openURLIntent, PackageManager.GET_ACTIVITIES);
					if (list.size() > 0) {
